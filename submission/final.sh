...
# WRITE YOUR SOLUTION BELOW:
P2SH_VALID=$(bitcoin-cli -regtest -rpcwallet=btrustwallet validateaddress "$P2SH_ADDR" | grep -w 'isvalid' | awk '{print $2}' | tr -d ',')
...
# WRITE YOUR SOLUTION BELOW:
INTERNAL_KEY=$(echo "$ADDR_INFO" | grep -w 'pubkey' | awk -F '"' '{print $4}')
...
# WRITE YOUR SOLUTION BELOW:
TAPROOT_DESCRIPTOR=$(bitcoin-cli -regtest getdescriptorinfo "$SIMPLE_DESCRIPTOR" | grep -w 'descriptor' | awk -F '"' '{print $4}' )
